...
module.exports = async (client, channel, guild) => {
    if (channel.type ==  Discord.ChannelType.GuildAnnouncement) {
        try {
            ...
            channelName = channelName.replace(`{name}`, `Canales de anuncios: ${guild.channels.cache.filter(channel => channel.type ===  Discord.ChannelType.GuildAnnouncement).size || 0}`)
